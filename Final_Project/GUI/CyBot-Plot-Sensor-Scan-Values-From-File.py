...
for line in file_data: 
    if(line!="END"):
        ...
        try:
            ...
            angle_degrees.append(float(re.sub(r'[a-zA-Z]', '', (data[0]))))  # Column 0 holds the angle at which distance was measured
            distance.append(float(re.sub(r'[a-zA-Z]', '', (data[1])))/100)       # Column 1 holds the distance that was measured at a given angle       
        except:
            pass
...
ax.tick_params(axis='both', which='major', labelsize=10) # set font size of tick labels
ax.set_rmax(1.5)                    # Saturate distance at 2.5 meters
ax.set_rticks([0.25, 0.5, 0.75, 1.0, 1.25,1.5])   # Set plot "distance" tick marks at .5, 1, 1.5, 2, and 2.5 meters
